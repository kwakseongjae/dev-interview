...
### 2026-03-17: [AI/Embedding] 메타 쿼리와 질문 콘텐츠의 임베딩 유사도 갭

- **실수**: 사용자 검색 쿼리("프론트엔드 3년차 면접")와 실제 질문 내용("React useMemo 설명해주세요")을 벡터 유사도로 직접 비교 → 최대 유사도 0.57로 임계값 미달
- **원인**: 메타 요청(의도)과 실제 콘텐츠(질문)는 시맨틱적으로 다른 카테고리에 속함. 임베딩 모델은 텍스트의 "의미"를 비교하지 "의도→결과" 매핑을 하지 않음
- **규칙**:
  - 검색 쿼리와 저장된 콘텐츠가 시맨틱적으로 다른 레벨이면 순수 벡터 검색은 실패함
  - 카테고리/키워드 기반 필터링 → 그 안에서 벡터 유사도 랭킹 (하이브리드 접근) 사용
  - 임베딩은 "같은 레벨의 텍스트 비교"에만 효과적 (질문↔질문, 쿼리↔쿼리)
- **참조**: #58, `src/lib/question-cache.ts`

---

### 2026-03-17: [API] Claude 모델 ID는 공식 문서에서 확인 필수

- **실수**: `claude-sonnet-4-6-20260218`이라는 존재하지 않는 스냅샷 ID를 사용. 리서치 에이전트가 잘못된 ID를 제공
- **원인**: 모델 ID를 추정으로 작성하고 공식 문서를 확인하지 않음
- **규칙**:
  - Claude 모델 ID 변경 시 반드시 `https://platform.claude.com/docs/en/about-claude/models/overview` 확인
  - alias(`claude-sonnet-4-6`)는 자동으로 최신 스냅샷으로 라우팅되므로 alias 사용 권장
  - 스냅샷 ID가 필요한 경우만 날짜 포함 ID 사용
- **참조**: #58, `src/lib/claude.ts`

---

### 2026-03-17: [Database] PostgREST RPC 함수의 vector 타입 파라미터 비호환

- **실수**: Supabase RPC 함수를 `vector(1024)` 파라미터로 정의했으나, PostgREST는 JS 클라이언트에서 `number[]`를 `float8[]`로 전달 → 함수 시그니처 매칭 실패 (PGRST202)
- **원인**: Supabase JS 클라이언트가 JSON 배열을 `vector` 타입으로 자동 캐스팅하지 않음
- **규칙**:
  - Supabase RPC에서 벡터를 받을 때 `float8[]` 타입으로 선언하고 함수 내부에서 `::vector(N)` 캐스팅
  - `vector` 타입을 직접 파라미터로 사용하면 PostgREST 타입 매칭 실패
- **참조**: #58, `search_similar_questions` → `find_similar_questions` 마이그레이션
...
### 2026-03-29: [Middleware] 미들웨어에 인증 상태 기반 DB 쿼리/리다이렉트 추가 금지

- **실수**: (1) 미들웨어에서 getUser 실패 시 무조건 탈퇴 리다이렉트 → 세션 만료에도 오탐지. (2) 이를 수정하며 미들웨어에서 매 요청마다 admin DB 쿼리(deleted_at 확인)를 추가 → 성능 문제
- **원인**: 미들웨어는 모든 요청에 실행되므로 DB 쿼리나 복잡한 조건부 리다이렉트에 부적합
- **규칙**:
  - 미들웨어는 세션 리프레시(getUser)만 담당, 추가 로직 금지
  - 사용자 상태 기반 리다이렉트는 API 응답 → 클라이언트 처리 패턴 사용
  - 예: `/api/auth/me`가 `{ deleted: true }` 반환 → 클라이언트에서 리다이렉트
- **참조**: #69, `src/lib/supabase/middleware.ts`, `src/lib/api.ts`
