...
export function selectQuestionsByQuery(query: string): QuestionTemplate[] {
  ...
  if (lowerQuery.includes('프론트엔드') || lowerQuery.includes('frontend') ||
      lowerQuery.includes('react') || lowerQuery.includes('vue') ||
      lowerQuery.includes('javascript') || lowerQuery.includes('타입스크립트')) {
    return FRONTEND_3YEARS_QUESTIONS;
  }

  if (lowerQuery.includes('백엔드') || lowerQuery.includes('backend') ||
      lowerQuery.includes('서버') || lowerQuery.includes('api') ||
      lowerQuery.includes('java') || lowerQuery.includes('python') ||
      lowerQuery.includes('node')) {
    return BACKEND_3YEARS_QUESTIONS;
  }

  if (lowerQuery.includes('cs') || lowerQuery.includes('기초') ||
      lowerQuery.includes('알고리즘') || lowerQuery.includes('자료구조') ||
...
export function createQuestionsFromTemplates(templates: QuestionTemplate[]): Question[] {
